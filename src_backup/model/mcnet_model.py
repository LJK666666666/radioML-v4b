...
def build_simplified_mcnet_model(input_shape, num_classes):
    """
    Build a simplified version of MCNet with fewer blocks.
    
    Reduces complexity while maintaining the core multi-scale concept.
    ...
    # Initial processing
    x = Conv2D(32, kernel_size=(3, 7), padding='same', activation='relu',
              name='conv0', kernel_initializer='glorot_normal')(x)
    x = MaxPooling2D(pool_size=(1, 2), strides=(1, 2), padding='valid')(x)
    
    # Simplified pre-block
    x = pre_block(x, conv1_size=(1, 3), conv2_size=(3, 1),
                 pool_size=(1, 2), block_name='pre_block')
    
    # Single M-block
    x = m_block(x, filters_size01=16, filters_size02=24, filters_size03=16,
               conv0_size=(1, 1), conv1_size=(1, 3), conv2_size=(3, 1),
               conv3_size=(1, 1), block_name='m_block')
    ...
    model = Model(inputs=inputs, outputs=outputs, name='MCNet_Simplified')
...
def MCNet(input_shape, num_classes):
    """
    Legacy function name for MCNet model building.
    ...
    Returns:
        Compiled MCNet model
    """
    return build_mcnet_model(input_shape, num_classes)